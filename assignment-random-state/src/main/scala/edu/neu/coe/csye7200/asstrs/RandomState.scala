...
trait RandomState[T] {
  ...
  def toStream: LazyList[T] = {
    def inner(xs: RandomState[T]): LazyList[T] = xs.get #:: inner(xs.next)
    inner(this)
  }
}
...
case class JavaRandomState[T](n: Long, g: Long => T) extends RandomState[T] {
  ...
  // 7 points
  def next: RandomState[T] = JavaRandomState(r.nextLong(),g)
...
object RandomState {
  ...
  // 4 points
  val longToDouble: Long => Double = {x => x.asInstanceOf[Double] / Long.MaxValue.asInstanceOf[Double]}
